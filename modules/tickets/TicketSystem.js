    ...
    client.on('interactionCreate', async interaction => {
        if (!interaction.isButton()) return;
            if(interaction.message.channel.id == TicketChannelID || interaction.message.channel.id == OrderChannelID || interaction.message.channel.id == RecoverChannelId){
                interaction.reply({content: "<a:check:939238439826640957> **Ping Successful!** You can now Open a Ticket!", ephemeral: true})
            }
    })
    client.on('interactionCreate', async interaction => {
        ...
        if (interaction.message.channel.id == TicketChannelID) {
            ...
            if(menuIndex){
                ...
                switch(menuIndex.type){
                    case "HELP" : {
                        await interaction.reply({content: `<:koojedevelopment:937113253945933834> Creating your Ticket ... `, ephemeral: true})
                    ...
                    case "TEAMAPPLY" : {
                        await interaction.reply({content: `<:Editing:877653386747605032> Creating your Ticket ... `, ephemeral: true})
                        Cooldown.set(user.id, Date.now())
                        setTimeout(()=>Cooldown.delete(user.id), cooldownamount);
                        ////////////////////////
                        /////////////////////////
                    ...
                    case "PARTNERAPPLY" : {
                        await interaction.reply({content: `<:Partner:866089513436315698> Creating your Ticket ... `, ephemeral: true})
                        Cooldown.set(user.id, Date.now())
                        setTimeout(()=>Cooldown.delete(user.id), cooldownamount);
                        ////////////////////////
                        /////////////////////////
