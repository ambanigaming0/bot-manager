...
module.exports = client => {
    client.on("ready", () => {
        let milratoGuild = client.guilds.cache.get("934213686468423780")
        ...
        app.get("/staff", checkAuth, async (req, res) => {
            ...
            if (!member) return res.redirect("/?error=true&message=" + encodeURIComponent("I don't have any Data of you, try asking Protyo about it."));
        ...
        app.get("/createbot", checkAuth, async (req, res) => {
            ...
            if (!member.permissions.has("ADMINISTRATOR") && !member.roles.cache.has("935689219458863214") && !member.roles.cache.has("934221436564930610") && !member.roles.cache.has("831972877480755251"))
                return res.redirect("/?error=true&message=" + encodeURIComponent("This site is not meant for you! :)"));
        ...
        app.post("/createbot", checkAuth, async (req, res) => {
            try {
                ...
                let BotType = "Default";
                let BotDir = "Default";
                if (ticketChannel.parentId == "938418981197451294") {
                    BotType = "System Bot";
                    BotDir = "SYSTEMBOTS";
                }
                if (ticketChannel.parentId == "938439892638257172") {
                    BotType = "Music Bot";
                    BotDir = "MusicBots";
                }
                ...
                //Check for ERRORS
                if (owner.length < 17 || owner.length > 19) return res.redirect("/createbot?error=true&message=" + encodeURIComponent("Invalid Owner ID, that would be a valid example: `717416034478456925`"))
                    ...
                    client.users.fetch(owner).then(user => {
                        ...
                        user.send({
                            ...
                            embeds: [new Discord.MessageEmbed().setColor(client.config.color).addField("📯 Invite link: ", `> [Click here](https://discord.com/oauth2/authorize?client_id=${botuser.id}&scope=bot&permissions=8)`)
                                .addField("💛 Support us", `> **Please give us <#937678247011954738> and stop at <#941439058629001246> so that we can continue hosting Bots!**`).setTitle(`\`${botuser.tag}\` is online and ready to be used!`).setDescription(`<@${botuser.id}> is a **${BotType}** and got added to: <@${owner}> Wallet!\nTo get started Type: \`${prefix}help\``).setThumbnail(botuser.displayAvatarURL())
                            ]
                        }).catch(() => { });
                    }).catch(() => { });
                } catch (e) {
         ...
         */
         try {
            ...
            const key = fs.readFileSync("/etc/letsencrypt/live/dash.kooje.me/privkey.pem", `utf8`);
            const cert = fs.readFileSync("/etc/letsencrypt/live/dash.kooje.me/fullchain.pem", `utf8`);
            const ca = fs.readFileSync("/etc/letsencrypt/live/dash.kooje.me/chain.pem", `utf8`);
